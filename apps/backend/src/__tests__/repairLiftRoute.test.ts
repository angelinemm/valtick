...
describe.skipIf(!HAS_DB)("POST /repair_lift", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    const res = await request(app).post("/api/repair_lift").send({ liftId: brokenLiftId });
  ...
  it("lift belonging to a different resort: resort unchanged", async () => {
    ...
    const res = await agent.post("/api/repair_lift").send({ liftId: otherResort.lifts[0].id });
    expect(res.body.resort.moneyCents).toBe(5000);
